...
var headmates = [
    ...
    },
    {
        ...
        role: "",
        firstNoted: "August 30, 2026",
        gender: "",
        age:"Ageless",
        quote: "",
        species: "anthro tiger",
        origin:"Brainmade",
        misc:`faceclaim is tigress from Kung fu Panda`,
        links:``,
        pictures: `no pic yet...`,
    },
    {
...
function getProjs() {
    if (seltags.length >= 1) { //if tags are selected display only those 
         deleteAllProj();
         addTaggedProjs();
...
function addTaggedProjs(){ // *works*
    for(let i=0; i< seltags.length; i++){ //for if there is multiple tags selected
        ...
        for(let i=0; i<projects.length; i++){
            if(projects[i].projtags.includes(tagName)){
                addToShow(i); 
